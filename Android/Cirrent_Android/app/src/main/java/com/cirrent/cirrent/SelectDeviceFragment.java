...
public class SelectDeviceFragment extends Fragment {
            ...
            if (device.isIdentifyingActionEnabled() == true) {
                ...
                identifyActionButton.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        CirrentService.sharedService().identifyYourself(SampleCloudService.sharedService().search_token, device.getDeviceId(), new CompletionHandler() {
                            @Override
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        CirrentService.sharedService().pollForUserAction(SampleCloudService.sharedService().search_token, new CompletionHandler(){
            @Override
    ...
    void selectDeviceAndAskStatus(final Device device) {
        ...
        SampleCloudService.sharedService().bindDevice(device.getDeviceId(), device.getFriendlyName(), new SampleCloudService.SampleCompletionHandler(){
            ...
            public void completionBindDevice(boolean bSuccess) {
                if (bSuccess == true) {
                    CirrentService.sharedService().bindDevice(SampleCloudService.sharedService().bind_token, device, new CompletionHandler(){
                        @Override
                        public void completion(CirrentType.RESPONSE response) {
                            if (response != CirrentType.RESPONSE.SUCCESS) {
                                ...
                                SampleCloudService.sharedService().getDevices(new SampleCloudService.SampleCompletionHandler(){
                                    @Override
                                    public void completionGetDevices(ArrayList<SampleCloudService.SampleDevice> devices) {
                                        if (devices != null && devices.size() > 0) {
