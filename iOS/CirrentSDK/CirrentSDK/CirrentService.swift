...
public class CirrentService : NSObject {
    ...
    @objc public var model:Model? = nil
    ...
    @objc public func supportSoftAP(bSupport:Bool) {
    ...
    /// - Returns: OwnerID
    @objc public func setOwnerIdentifier(identifier:String = "") {
        if identifier == "" {
    ...
    @objc private func appDidBecomeActive() {
        ...
        if diff > SOFTAP_FIND_LIMIT && (ssid == nil || ssid!.contains((model?.SoftAPSSID)!) == false) {
            LogService.sharedService.log(event: .SoftAP_LONG_DURATION, data: "")
        }
        
        if ssid == nil {
            LogService.sharedService.log(event: .SoftAP_ERROR, data: "wanted-ssid=\(String(describing: model?.SoftAPSSID));got-ssid=nil;timeout=\(diff)")
            LogService.sharedService.putLog(token: nil)
        }
        
        if ssid!.contains((model?.SoftAPSSID)!) == false {
            LogService.sharedService.log(event: .SoftAP_ERROR, data: "wanted-ssid=\(String(describing: model?.SoftAPSSID));got-ssid=\(ssid!);timeout=\(diff)")
            LogService.sharedService.putLog(token: nil)
        }
    }
    ...
    @objc public func deleteNetwork(tokenMethod:TokenHandler, deviceID:String, network:Network, completion: @escaping CompletionHandler) {
    ...
    @objc public func addNetwork(tokenMethod:TokenHandler, deviceID:String, network:Network, password:String, completion: @escaping CompletionHandler) {
        tokenMethod(.MANAGE, deviceID, {
            manageToken in
    ...
    @objc public func putPrivateCredentials(tokenMethod:TokenHandler, deviceID:String, bAddToVault:Bool = true, completion: @escaping CredentialCompletionHadler) {
        tokenMethod(.MANAGE, deviceID, {
            manageToken in
            ...
            if (self.model?.GCN)! {
                ...
                if ssid.contains((self.model?.SoftAPSSID)!) != true {
    ...
    @objc public func getDeviceJoiningStatus(tokenMethod:TokenHandler, deviceID:String, handler: @escaping JoiningHandler) {
        tokenMethod(.MANAGE, deviceID, {
            manageToken in
            ...
            self.joiningTimer = Timer.scheduledTimer(withTimeInterval: Constants.JOINING_TIME_INTERVAL, repeats: true, block: {
                ...
                if self.model?.GCN == true {    // checking status in the cloud
                    ...
                    self.getDeviceJoiningStatusfromCloud(manageToken:manageToken!, handler: {
                        ...
                        // BARBARA - to clean up - which statuses should kill the timer and which ones don't
                ...
                else {  // checking status over SoftAP
                    ...
                    if ssid != nil && ssid!.contains((self.model?.SoftAPSSID)!) != true {
                        ...
                        self.getDeviceJoiningStatusfromCloud(manageToken:manageToken!, handler: {
                            ...
                            if index > waitTime {
                                t.invalidate()
                                LogService.sharedService.debug(data: "JOINING - Failed - TIMED_OUT")
                                let logStr = "ssid=\((self.model?.ssid)!);credentialID=\((self.model?.credentialId!)!)"
                                LogService.sharedService.log(event: .CREDS_TIMEOUT, data: logStr)
                                self.needSoftAP()
                                handler(JOINING_STATUS.TIMED_OUT)
                                return
                            }
    ...
    @objc public func processSoftAP(handler: @escaping SoftAPHandler) {
        ...
        if model != nil && model?.ssid != nil && model?.ssid!.contains((model?.SoftAPSSID)!) == true {
            ...
            SoftAPTimer = Timer.scheduledTimer(withTimeInterval: Constants.SoftAP_TIME_INTERVAL, repeats: true, block: {
                ...
                if ssid == nil || ssid!.contains((self.model?.SoftAPSSID)!) == false {
...
// Private Methods
    ...
    @objc public func getCurrentSSID(bLog:Bool = false) -> String? {
