...
protocol ATAboutViewDelegate {
    func okButtonActionAbout()
}

class ATAboutView: UIView {
    
    var delegate: ATAboutViewDelegate?
...
//        attachment.bounds = CGRect(x: CGFloat(0), y: offsetY, width: CGFloat((attachment.image?.size.width)!), height: CGFloat((attachment.image?.size.height)!))
        ...
        let myString1 = NSMutableAttributedString(string: " " + NSLocalizedString("About", comment: ""))
        myString.append(myString1)
        print("\(myString.length)")
        myString.setAttributes([NSFontAttributeName:titleLabel.font!,NSBaselineOffsetAttributeName:5], range: NSRange(location:1,length:(NSLocalizedString("About", comment: "").characters.count)+1))
        ...
        var sum = 0
        let x:Int = 10
        var y:Int = Int(titleLabel.bounds.size.height) + 30
        ...
        for i in 1...count {
            ...
            if i == 1 {
                messagedLabel.text = "Application name : Samsung ARTIK"
            }
            else if i == 2 {
                
                let build = Bundle.main.infoDictionary?[kCFBundleVersionKey as String] as! String

                messagedLabel.text = "Application version : \(build)"
            }
        ...
        confirmButton.addTarget(self, action: #selector(okButtonActionAbout), for: UIControlEvents.touchUpInside)
    ...
    func okButtonActionAbout() {
        self.delegate?.okButtonActionAbout()
    }
