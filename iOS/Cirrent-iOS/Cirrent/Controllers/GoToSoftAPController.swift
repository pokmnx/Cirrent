...
class GoToSoftAPController: BaseViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        let softAPSSID:String = (CirrentService.sharedService.model?.SoftAPSSID)!
        shouldSelectSoftAPComment.text = "Select \(softAPSSID)"
        SoftAPSSIDLabel.text = CirrentService.sharedService.model?.SoftAPSSID
    ...
    func checkSoftAPAndGoDirectly() -> Bool {
        ...
        if currentSSID!.contains((CirrentService.sharedService.model?.SoftAPSSID)!) == true {
            return true
        }
        ...
        CirrentService.sharedService.processSoftAP(handler: {
            ...
            DispatchQueue.main.async {
                ...
                case .SUCCESS_WITH_SoftAP:
                    let deviceID = CirrentService.sharedService.model?.selectedDevice?.getDeviceID()
                    let verified = self.verifyAccountID(deviceID: deviceID!)
    ...
    func verifyAccountID(deviceID:String) -> Bool {
        let accountID = SampleCloudService.sharedService.accountID
        if accountID == nil {
            return false
        }
        
        let range = deviceID.range(of: accountID!)
        if range == nil {
            return false
        }
